...
## Follow-up: shared game fitting and new-project templates — September 3

The earlier Burp and Flappy changes handled those starters' object placement. The shared preview now gives every ordinary game a stable playfield and scales it proportionally to fit its panel. When a game omits its design dimensions, the wrapper takes them from the initial viewport and accounts for its pixel scale. Authored dimensions remain intact. Existing saved games receive this behavior when run without changing their stored source. The same helper is embedded in downloaded HTML games, so exporting keeps the sizing behavior.

Both the single-file example template and the multi-file project template now explicitly use a 960 × 540 design with KAPLAY's letterboxing enabled. This makes new games portable across starting window sizes. The physical canvas continues filling the available panel, while the game's coordinates stay stable; matching the aspect ratio can leave space around the playfield. Explicit `letterbox: false`, stretched sizing, and custom canvas/root options remain under the author's control.

The extended tests also exposed a shared panel timing problem. Allotment could change the iframe size while the canvas's resize observer was processing an earlier measurement, producing “ResizeObserver loop completed with undelivered notifications.” GameView now measures its container and applies the final iframe dimensions once per animation frame. The game remains mounted, and no resize errors are hidden or filtered out.

| Check | Result |
| --- | --- |
| Full sample catalog | All 127 samples started and passed resizing at 1042 × 600, 390 × 700, 1440 × 900, and 1042 × 936: 508 catalog resize checks. Canvas dimensions matched their frames within 2 px, design coordinates stayed stable, and resizing produced no runtime errors. |
| Templates and focused fixtures | Both new templates retained their 960 × 540 design at all four sizes. Additional checks covered an implicitly sized game, its actual HTML export, and an explicitly responsive game. Including the 16 existing game/regression starters, the combined matrix contains 148 audits and 592 resize checks. |
| Pointer alignment | Native mouse input hit four targets near the design corners after each resize, in both the preview and the exported HTML: 32 successful clicks. The fixture used pixel density 2, and the final test waited for the drawing buffer to match the new canvas before clicking. |
| State preservation | Resizing retained the iframe, preview run, project generation, and source revision. Paused object positions were compared for the templates and playable starters. No saved-project migration or source rewrite is required. |
| Authored sizing | The browser check confirmed that `letterbox: false` still allows logical dimensions to follow the canvas. Unit checks covered authored portrait dimensions, non-default pixel scales, missing dimensions, initially hidden containers, custom canvas/root options, and standalone embedding of the helper. |
| Prompts and existing behavior | All 508 copyable catalog prompts matched their selected sample and idea. The eight WebMCP tools, persistence, sound controls, Project Browser motion, reduced motion, fixed tips height, and panel restoration remained covered by the passing integration suite. |
| Native browser: Runner | The result screen stayed centered as the panel changed from 834 × 434 to 894 × 434 and then 894 × 634 px when tips were minimized. The score remained 171 throughout. This sample received no individual source patch. |
| Native browser: fresh project | A new project created through the Project Browser stayed centered and fully visible at 894 × 434, 734 × 434, and 1278 × 674 px, including full workspace expansion. Screenshots were visually reviewed against the hosted sandbox. |
| Release validation | `npm run verify:webmcp` passed 74 unit tests in 15 suites, TypeScript, browser integration, and both builds. `npm run test:catalog` passed the complete catalog audit. `git diff --check` passed. |

The test harness now captures CSS transitions before a busy frame can finish them and waits for the engine's resized viewport or drawing buffer before inspecting or clicking. Its library fixture also supports the export fetch's CORS preflight, and a failed test stops waiting when its browser closes. These changes preserve the assertions while removing timing and fixture failures encountered during this work.

The catalog audit verifies startup, resizing, bounded scene geometry, state preservation, and prompt wiring; it is not a complete playthrough of every game. Geometry inspection covers up to 50 objects while paused. The corner-target fixture and the existing Flappy gameplay checks use live native input. Additional browser engines were not tested. [Shared game scaling evidence](2026-09-03/shared-game-scaling.json) records every resize result, final release checks, and native browser measurements.
